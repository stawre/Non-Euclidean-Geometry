...
MousePosCallback(GLFWwindow* window, double mouse_x, double mouse_y)
{
	if (!g_mouse_pressed)
		return;
	if (g_camera.get_button_pressed()) {
		g_camera.set_mouse(mouse_x, mouse_y);
		g_camera.set_button_pressed(false);
	}
	if (g_current_button == GLFW_MOUSE_BUTTON_LEFT) {
		// FIXME: left drag
		g_camera.set_pny(mouse_x, mouse_y);
	} else if (g_current_button == GLFW_MOUSE_BUTTON_RIGHT) {
		// FIXME: middle drag
		g_camera.set_zoom_mouse(mouse_y);
	} else if (g_current_button == GLFW_MOUSE_BUTTON_MIDDLE) {
		// FIXME: right drag
		g_camera.set_pan_mouse(mouse_x, mouse_y);
	}
}
...
{
	g_camera.set_button_pressed(true);
